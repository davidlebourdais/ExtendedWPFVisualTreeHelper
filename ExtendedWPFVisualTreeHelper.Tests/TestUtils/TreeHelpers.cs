...
namespace EMA.ExtendedWPFVisualTreeHelper.Tests.Utils
{
    ...
    public static class TreeHelpers
    {
        ...
        /// <param name="allow_content_element">Allow travelling through <see cref="ContentElement"/> objects.</param>
        /// <returns>The nammed element, if found, null otherwise.</returns>
        public static DependencyObject FindElementByName(DependencyObject node, string name, bool allow_content_element = true)
        {
            ...
            if (node is Visual || node is Visual3D)
            {
                ...
                {
                    result = FindElementByName(VisualTreeHelper.GetChild(node, i), name, allow_content_element);
                    if (result != null) return result;
                }
            }

            if (allow_content_element)
            {
                var contentElements = LogicalTreeHelper.GetChildren(node).OfType<ContentElement>();
                foreach (var child in contentElements)
                {
                    result = FindElementByName(child, name, allow_content_element);
                    if (result != null) return result;
                }
            }
        ...
        /// <param name="allow_content_element">Allow travelling through <see cref="ContentElement"/> objects.</param>
        /// <returns>The nammed element, if found, null otherwise.</returns>
        public static DependencyObject FindDirectElementByName(DependencyObject node, string name, bool allow_content_element = true)
        {
            if (node == null) return null;
            ...
            if (result == null)
            {
                ...
                if (count > 0)
                    result = FindDirectElementByName(VisualTreeHelper.GetChild(node, 0), name, allow_content_element);
                else if (allow_content_element)
                {
                    var contentElements = LogicalTreeHelper.GetChildren(node).OfType<ContentElement>();
                    if (contentElements.FirstOrDefault() is DependencyObject asDO)
                        result = FindDirectElementByName(asDO, name, allow_content_element);
                }
            }
        ...
        public static int GetElementDepthByName(DependencyObject node, string name, bool allow_content_element = true)
        {
            var element = FindElementByName(node, name, allow_content_element);
        ...
        public static List<DependencyObject> FindVisualChildren(DependencyObject node, bool allow_content_element = true)
        {
        ...
        public static List<DependencyObject> FindAllVisualChildren(DependencyObject node, bool allow_content_element = true)
        {
            if (node == null)
                return new List<DependencyObject>();

            var results = FindVisualChildren(node, allow_content_element);
            ...
            while (current_count > 0)
            {
                ...
                {
                    var results_tmp = FindVisualChildren(results[i], allow_content_element);
